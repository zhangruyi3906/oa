...
import com.lh.oa.module.system.dal.mysql.dict.DictDataMapper;
...
public class DictDataServiceImpl implements DictDataService {
    ...
    @Override
    public List<DictDataBatchVo> batchQueryDictData(String dictTypeList) {
        List<DictDataDO> dictList = dictDataMapper.selectListByDictTypes(Arrays.asList(dictTypeList.split(",")));
        Map<String, List<DictDataDO>> groupByDictType = dictList.stream().collect(Collectors.groupingBy(DictDataDO::getDictType));
        List<DictDataBatchVo> result = new LinkedList<>();
        groupByDictType.forEach((dictType, dictDos) -> {
            DictDataBatchVo batchVo = new DictDataBatchVo();
            batchVo.setDictType(dictType);
            List<DictDataBaseVO> dictDataVOS = JsonUtils.parseArray(JsonUtils.toJsonString(dictDos), DictDataBaseVO.class);
            batchVo.setDataVoList(dictDataVOS.stream().sorted(Comparator.comparing(DictDataBaseVO::getSort)).collect(Collectors.toList()));
            result.add(batchVo);
        });
        return result;
    }
