...
public interface ErrorCodeConstants {
    ...
    ErrorCode USER_NOT_LOGIN = new ErrorCode(1002000002, "用户未登录或登录已过期，请重新登录");
    ...
    ErrorCode USER_HIRE_DATE_NOT_EXIST = new ErrorCode(1002003009, "用户入职时间不存在，请确认后再查询");

    // ========== 部门模块 1002004000 ==========
    ErrorCode DEPT_NAME_DUPLICATE = new ErrorCode(1002004000, "已经存在该名字的部门");
    ErrorCode ORGANIZATION_NOT_EXITS = new ErrorCode(1002004010, "部门必须要有父级组织");
    ...
    ErrorCode DEPT_NOT_ENABLE = new ErrorCode(1002004006, "上级部门不处于开启状态，不允许选择");
    ErrorCode DEPT_PARENT_IS_CHILD = new ErrorCode(1002004007, "不能设置自己的子部门为父部门");
    ErrorCode DEPT_HAS_CHILD = new ErrorCode(1002004008, "子部门未全部禁用");
    ErrorCode DEPT_EXISTS_USER_CHILD = new ErrorCode(1002004009, "部门或子部门的员工未被禁用");
    ...
    //==================考勤====================
    ErrorCode RECORD_NOT_EXISTS = new ErrorCode(1002029000, "打卡记录不存在");
//    ErrorCode USER_PROJECT_NOT_EXISTS = new ErrorCode(1002029000, "打卡记录不存在");
    ErrorCode ATTENDANCE_RULE_NOT_EXISTS = new ErrorCode(1002029001, "打卡规则（部门）不存在");
    ErrorCode ATTENDANCE_RULE_IS_EXISTS = new ErrorCode(1002029002, "打卡规则（部门）已经存在");
    ErrorCode PROJECT_ATTENDANCE_RULE_NOT_EXISTS = new ErrorCode(1002029003, "打卡规则（项目）不存在");
    ErrorCode PROJECT_ATTENDANCE_RULE_IS_EXISTS = new ErrorCode(1002029004, "打卡规则（项目）已经存在");
    ErrorCode ATTENDANCE_ROUND_NOT_EXISTS = new ErrorCode(1002029005, "超出打卡范围");
    ErrorCode HOLIDAY_NOT_RECORD = new ErrorCode(1002029006, "休息日请勿打卡");
    ErrorCode ATTENDANCE_RULE_DEPT_PROJECT_IS_EXISTS = new ErrorCode(1002029007, "打卡规则（规则名/部门/项目）已经存在");
    ErrorCode ATTENDANCE_RULE_NAME_IS_EXISTS = new ErrorCode(1002029008, "存在相同规则名");

    //================项目管理========================
    ErrorCode USER_PROJECT_NOT_EXISTS = new ErrorCode(1002030000, "人员项目不存在");

    ErrorCode USER_PROJECT_IS_EXISTS = new ErrorCode(1002030001, "人员项目已存在");
    ErrorCode PROJECT_NAME_IS_EXISTS = new ErrorCode(1002030002, "项目名已存在");
    ErrorCode LEAVE_TIME_BEFORE_IN_TIME = new ErrorCode(1002030003, "离场时间不能小于进场时间");
    ErrorCode HIRE_DATE_NOT_EXISTS = new ErrorCode(1002030004, "用户入职时间不存在，请添加入职时间后再提交");
    ErrorCode IN_TIME_BEFORE_HIRE_DATE = new ErrorCode(1002030005, "进场时间不能小于入职时间");

    ErrorCode INFORMATION_NOT_EXISTS = new ErrorCode(1002040000, "员工信息不存在");
    ErrorCode INFORMATION_IS_EXISTS = new ErrorCode(1002040001, "员工已经存在");


    ErrorCode JOB_LEVEL_SALARY_NOT_EXISTS = new ErrorCode(1002050000, "员工工种等级基础工资不存在");
    ErrorCode DEPT_SALARY_NOT_EXISTS = new ErrorCode(1002050001, "部门员工考勤惩罚不存在");
    ErrorCode DEPT_SALARY_IS_EXISTS = new ErrorCode(1002050002, "部门员工考勤惩罚已存在");

    ErrorCode JOB_COMMISSION_NOT_EXISTS = new ErrorCode(1002060000, "项目工种提成不存在");

    ErrorCode PROJECT_WORK_TYPE_NOT_EXISTS = new ErrorCode(1002070000, "项目工种不存在");

    ErrorCode VOLUME_STATISTICS_NOT_EXISTS = new ErrorCode(1002080000, "员工方量统计不存在");


    ErrorCode SALARY_SETTLEMENT_NOT_EXISTS = new ErrorCode(1002090000, "员工工资结算不存在");


    ErrorCode MONTH_STATISTICS_NOT_EXISTS = new ErrorCode(1002100000, "考勤月统计不存在");


    ErrorCode SCHEDULE_NOT_EXISTS = new ErrorCode(1002110000, "日程不存在");

    ErrorCode SCHEDULE_IS_EXISTS = new ErrorCode(1002110001, "日程冲突，当前时间已存在其他日程");
    ErrorCode SCHEDULE_END_NOT_BEFORE_START = new ErrorCode(1002110002, "结束时间不能小于开始时间");

    ErrorCode SCHEDULE_NOT_BEFORE_NOW = new ErrorCode(1002110003, "开始时间不能小于当前时间");
    ErrorCode SCHEDULE_START_TIME_NOT_EXIST = new ErrorCode(1002110004, "日程开始时间不存在");
    ErrorCode SCHEDULE_END_TIME_NOT_EXIST = new ErrorCode(1002110005, "日程结束时间不存在");

    ErrorCode MEETING_NOT_EXISTS = new ErrorCode(1002120000, "会议组织不存在");

    ErrorCode MEETING_IS_EXISTS = new ErrorCode(1002120001, "会议室已被预约");
    ErrorCode ORDER_MEET_STATUS_ERROR = new ErrorCode(1002120003, "预约状态异常");
    ErrorCode MEET_NAME_IS_EXISTS = new ErrorCode(1002120002, "会议室名已存在");

    ErrorCode AFTER_APPROVAL_ON_FURTHER_MODIFICATIONS_CAN_BE_MADE = new ErrorCode(1002120004, "审核通过后不能再修改");

    ErrorCode AUDIT_IS_EXISTS = new ErrorCode(1002120002, "预约已被审核");

    ErrorCode MEETING_CONTENT_NOT_EXISTS = new ErrorCode(1002130000, "会议记录不存在");
    ErrorCode MEET_ROOM_IS_USED = new ErrorCode(1002130001, "会议室已被使用");

    ErrorCode PROJECT_NOT_EXISTS = new ErrorCode(1002150000, "项目不存在");
    ErrorCode CUSTOMERS_PROJECT_NOT_EXISTS = new ErrorCode(1002150001, "该客户不存在项目");

    ErrorCode CONTRACT_IS_EXISTS = new ErrorCode(1002150002, "已存在重复合同名");

    ErrorCode CUSTOMERS_NOT_EXISTS = new ErrorCode(1002160000, "客户基础信息不存在");

    ErrorCode CUSTOMERS_NAME_IS_EXISTS = new ErrorCode(1002160000, "已存在相同客户名");
    ErrorCode WORK_LOG_NOT_EXISTS = new ErrorCode(1002170000, "日志不存在");
    ErrorCode WORK_LOG_IS_EXISTS = new ErrorCode(1002170001, "当天日志已经存在");
    ErrorCode WORK_LOG_CONTENT_CONTAIN_FILE = new ErrorCode(1002170002, "微博内容不能包含文件或图片");
    ErrorCode WORK_LOG_NOT_UPDATE = new ErrorCode(1002170004, "日志时间超过七天无法编辑");

    ErrorCode WORK_LOG_NOT_REPAIR = new ErrorCode(1002170005, "日志时间超过七天天无法补交");
    ErrorCode WORK_LOG_is_ROLE = new ErrorCode(1002170002, "无权限");
    ErrorCode CUSTOMER_SERVICE_NOT_EXISTS = new ErrorCode(1002181000, "客户服务不存在");
    ErrorCode START_AFTER_END = new ErrorCode(1002182000, "开始时间大于结束时间");



    ErrorCode BUDGET_CAPITAL_NOT_EXISTS = new ErrorCode(1002190000, "资金预算不存在");
    ErrorCode BUDGET_CAPITAL_NOT_ENOUGH = new ErrorCode(1002190001, "资金预算不够");

    ErrorCode FUND_TRANSFER_NOT_EXISTS = new ErrorCode(1002190002, "资金划拨不存在");


    ErrorCode CAR_NOT_EXISTS = new ErrorCode(1002110100, "车辆不存在");
    ErrorCode CAR_CODE_NOT_EXISTS = new ErrorCode(1002110101, "请填入车牌号");
    ErrorCode CAR_PLATE_NUMBER_EXIST = new ErrorCode(1002110102, "已存在相同车牌号");
    ErrorCode CAR_STATUS_NOT_FREE = new ErrorCode(1002110103, "车辆被借用中，不能删除");
    ErrorCode CARBOOROW_NOT_EXISTS = new ErrorCode(1002110200, "车辆借用申请不存在");
    ErrorCode CAR_BORROW_IS_EXISTS = new ErrorCode(1002110201, "车辆已被借用");
    ErrorCode CAR_BORROW_TIME_ERROR = new ErrorCode(1002110202, "借用时间或者预计归还时间有误");
    ErrorCode CAR_IS_RETURN = new ErrorCode(1002110203, "车辆已归还");
    ErrorCode USERID_IS_ERROR = new ErrorCode(1002110204, "只能归还自己借的车");
    ErrorCode CAR_BORROW_IS_NOT_PENDING = new ErrorCode(1002110205, "车辆借用申请不属于待审核");
    ...
    ErrorCode PUBLIC_FILE_PARAMETER_NOT_EXIST = new ErrorCode(1001000005, "公共文件参数未选择");
    ...
    // ========== 工资条 1001007000 ==========
    ErrorCode SALARY_IMPORT_LIST_IS_EMPTY = new ErrorCode(1001008001, "导入工资数据不能为空！");
    ErrorCode THIS_MONTH_SALARY_LIST_IS_EXIST = new ErrorCode(1001008002, "当月已存在工资数据，不能导入！");
    ErrorCode CANNOT_LOOK_OTHER_SALARY_LIST = new ErrorCode(1001008003, "手机号与录入数据不匹配，请确认后再提交");
    ErrorCode SALARY_IMPORT_MONTH_IS_EMPTY = new ErrorCode(1001008004, "导入工资月份不能为空！");
    ErrorCode SALARY_IMPORT_MONTH_IS_ERROR = new ErrorCode(1001008005, "导入工资月份数据异常，请确认后在提交！");
