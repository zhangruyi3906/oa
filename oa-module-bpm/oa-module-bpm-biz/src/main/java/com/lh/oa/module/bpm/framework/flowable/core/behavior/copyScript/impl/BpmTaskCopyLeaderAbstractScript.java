 ...
 */
@Slf4j
public abstract class BpmTaskCopyLeaderAbstractScript implements BpmTaskCopyScript {
    ...
    protected Set<Long> calculateTaskCopyUsers(Task task, int level) {
    ...
    public Set<Long> getPartyCopyeeUser(Object party, int level) {
        if (party instanceof Integer) {
            ...
            Set<Long> CopyeeUserIds = new HashSet<>();
            for (Integer partyUserId : partyUserIds) {
                DeptRespDTO dept = getLevelDept(partyUserId.longValue(), level);
                if (Objects.nonNull(dept) && dept.getLeaderUserId() != null) {
                    CopyeeUserIds.add(dept.getLeaderUserId());
                }
            }
            return CopyeeUserIds;
    ...
    public Set<Long> getTransferDeptCopyeeUser(Object transferDept, int level) {
        if (transferDept instanceof Integer) {
            ...
            throw new BusinessException("调动部门数据格式错误，分配抄送人失败");
        }
    }
