...
public interface BpmTaskAssignRuleConvert {
    ...
    default List<BpmTaskKeyVO> convertBpmTaskKeyVOList(List<UserTask> tasks, List<BpmTaskAssignRuleDO> rules) {
        Map<String, BpmTaskAssignRuleDO> ruleMap = CollectionUtils.convertMap(rules, BpmTaskAssignRuleDO::getTaskDefinitionKey);
        // 以 UserTask 为主维度，原因是：流程图编辑后，一些规则实际就没用了。
        return CollectionUtils.convertList(tasks, task -> {
            BpmTaskKeyVO bpmTaskKeyVO = new BpmTaskKeyVO();
            bpmTaskKeyVO.setTaskDefinitionKey(task.getId());
            bpmTaskKeyVO.setTaskDefinitionName(task.getName());
            return bpmTaskKeyVO;
        });
    }
